...
// Created on: Jan-2019
// Created by: Brody MacNeil and Bradley wills
// Created for: ICS3U
...
class MainMenuScene: SKScene, SKPhysicsContactDelegate {
    // local variables to this scene
    let mainMenuSceneBackground = SKSpriteNode(imageNamed: "IMG_1720.JPG")
    let player1Button = SKSpriteNode(imageNamed: "IMG_5136.PNG")
    let logoImage = SKSpriteNode(imageNamed: "IMG_5263.PNG")
    let player2Button = SKSpriteNode(imageNamed: "IMG_5137.PNG")
    let settingsButton = SKSpriteNode(imageNamed: "IMG_5138.PNG")
    let statsButton = SKSpriteNode(imageNamed: "IMG_5139.PNG")
    ...
    override func didMove(to view: SKView) {
        ...
        logoImage.position = CGPoint(x: frame.midX, y: frame.midY + 125)
        logoImage.name = "logo"
        ...
        statsButton.position = CGPoint(x: frame.midX + 200, y: frame.midY - 100)
        statsButton.name = "stats button"
        self.addChild(statsButton)
        statsButton.setScale(0.11)
    }
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if let nodeTouchedName = nodeTouched.name{
            if nodeTouchedName == "stats button" {
...
class SettingsScene : SKScene, SKPhysicsContactDelegate{
    // local variables to this scene
    let settingsBackground = SKSpriteNode(imageNamed: "IMG_1720.JPG")
    let backButton = SKSpriteNode(imageNamed: "BlueArrow.png")
    let settingsLabel = SKLabelNode(fontNamed: "chalkduster")
    let musicLabel = SKLabelNode(fontNamed: "chalkduster")
    let onButton = SKSpriteNode(imageNamed: "IMG_5261.PNG")
    let offButton = SKSpriteNode(imageNamed: "IMG_5262.PNG")
    var musicButtons: Bool? = nil
    ...
    override func didMove(to view: SKView) {
        ...
        musicToggle = UserDefaults.standard.bool(forKey: "music toggle")
        
        settingsBackground.name = "settings Background"
        settingsBackground.position = CGPoint(x: frame.midX, y: frame.midY)
        settingsBackground.size = CGSize(width: frame.size.width, height: frame.size.height)
        self.addChild(settingsBackground)
        
        musicLabel.text = "Music"
        musicLabel.horizontalAlignmentMode = SKLabelHorizontalAlignmentMode.left
        musicLabel.fontColor = #colorLiteral(red: 1.0, green: 1.0, blue: 1.0, alpha: 1.0)
        musicLabel.fontSize = 50
        musicLabel.position = CGPoint(x: frame.midX, y: frame.midY)
        self.addChild(musicLabel)
        musicLabel.zPosition = 1.0
        
        settingsLabel.text = "Settings"
        settingsLabel.horizontalAlignmentMode = SKLabelHorizontalAlignmentMode.left
        settingsLabel.fontColor = #colorLiteral(red: 1.0, green: 1.0, blue: 1.0, alpha: 1.0)
        settingsLabel.fontSize = 50
        settingsLabel.position = CGPoint(x: frame.midX, y: frame.midY + 200)
        self.addChild(settingsLabel)
        settingsLabel.zPosition = 1.0
        
        backButton.position = CGPoint(x: frame.midX - 375 , y: frame.midY + 200)
        backButton.name = "back button"
        backButton.zRotation = CGFloat(M_PI_2)
        self.addChild(backButton)
        backButton.setScale(0.2)
        
        onButton.position = CGPoint(x: frame.midX + 200, y: frame.midY + 15)
        onButton.name = "on button"
        self.addChild(onButton)
        onButton.setScale(0.11)
        
    offButton.position = CGPoint(x: frame.midX + 275, y: frame.midY + 15)
    offButton.name = "off button"
    self.addChild(offButton)
    offButton.setScale(0.11)
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if let nodeTouchedName = nodeTouched.name{
            if nodeTouchedName == "back button" {
                self.settingsBackground.removeAllActions()
                let mainMenuScene = MainMenuScene(size: self.size)
                self.view!.presentScene(mainMenuScene)
            }
        }
        if let nodeTouchedName = nodeTouched.name{
            if nodeTouchedName == "on button" {
                onButton.color = UIColor.red
                onButton.colorBlendFactor = 1
                offButton.colorBlendFactor = 0
                musicButtons = true
                
                
                if musicButtons == true {
                    musicToggle = musicButtons!
                    UserDefaults().set(musicToggle, forKey: "music toggle")
                }

            }
        }
        if let nodeTouchedName = nodeTouched.name{
            if nodeTouchedName == "off button" {
                offButton.color = UIColor.red
                offButton.colorBlendFactor = 1
                onButton.colorBlendFactor = 0
                musicButtons = false
...
class StatsScene : SKScene, SKPhysicsContactDelegate{
    // local variables to this scene
    let settingsBackground = SKSpriteNode(imageNamed: "IMG_1720.JPG")
    let backButton = SKSpriteNode(imageNamed: "BlueArrow.png")
    let highScoreLabel = SKLabelNode(fontNamed: "avenir")
    let creditLabel = SKLabelNode(fontNamed: "avenir")
    ...
    override func didMove(to view: SKView) {
        ...
        creditLabel.text = "“Purple Planet Music | Energy.” Purple Planet Royalty Free Music, www.purple-planet.com/energy"
        creditLabel.horizontalAlignmentMode = SKLabelHorizontalAlignmentMode.left
        creditLabel.fontColor = #colorLiteral(red: 1.0, green: 1.0, blue: 1.0, alpha: 1.0)
        creditLabel.fontSize = 15
        creditLabel.position = CGPoint(x: frame.midX - 350, y: frame.midY - 100)
        self.addChild(creditLabel)
        creditLabel.zPosition = 1.0
    }
...
class MainGameScene : SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
        ...
        time.run(SKAction.repeatForever(SKAction.sequence([SKAction.wait(forDuration: 1), SKAction.run {
            ...
            if self.timeCounter > highScore {
                highScore = self.timeCounter
                UserDefaults().set(self.timeCounter, forKey: "highscore")
            }
            }])))
        ...
        if musicToggle == true {
            // plays music
            self.scene?.run(SKAction.repeatForever(SKAction.playSoundFileNamed ("Purple Planet Music - Energy - Deep In Trance.mp3", waitForCompletion: true)))
        }
    ...
    override func  update(_ currentTime: TimeInterval) {
        ...
        if !p1BikeTip.intersects(wallHitbox) {
            ...
            self.scene?.run(SKAction.sequence([SKAction.wait(forDuration: 2), SKAction.run {
                self.scene?.removeAllActions()
                let deathScene = DeathScene(size: self.size)
                self.view!.presentScene(deathScene)
                }]))
        }
    }
...
class DeathScene : SKScene, SKPhysicsContactDelegate {
    // local variables to this scene
    let Background = SKSpriteNode(imageNamed: "DeathSceneBackground.jpg")
    ...
    override func didMove(to view: SKView) {
        ...
        Background.size = CGSize(width: frame.size.width / 1.5, height: frame.size.height)
        self.addChild(Background)
        
        menuButton.position = CGPoint(x: frame.midX , y: frame.midY - 100)
        menuButton.name = "back button"
